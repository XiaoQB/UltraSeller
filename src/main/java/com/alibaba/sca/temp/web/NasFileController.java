...
public class NasFileController {
	...
	private void writeFile( StringBuilder  sb ) {
		try {
			...
			if (filePath.exists()) {
				sb.append( "writeFile NAS本地路径: " + nasPath ).append("，存在！").append("<br>");
				// 创建文件
        ...
        randomFile.write( (strTime + "SpringBoot服务写入！").getBytes("UTF-8") );   
        randomFile.close();   
			} else {
				sb.append( "NAS本地路径: " + nasPath + "不存在" ).append("<br>");
			}
		} catch (Exception e) {
	...
	private void readFile( StringBuilder  sb ) {
		sb.append( "readFile 读文件内容: " + nasPath + "/abc.txt" ).append("<br>");
		try {
      File filePath = new File( nasPath );
			// 创建文件
      String newFileName = filePath.getPath() + "/abc.txt";
